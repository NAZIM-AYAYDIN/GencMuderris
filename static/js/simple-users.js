...
// Tüm müderris formları için standart validation
function validateMuderrisForm(muderrisNumber) {
  const form = document.getElementById(`muderris${muderrisNumber}-form`);
...
// Şifre güçlülük kontrolünü iyileştir
function checkPasswordStrength(password) {
  let strength = 0;
  let feedback = [];
  ...
  if (password.length >= 8) {
    ...
    strength += 1;
    feedback.push('En az 8 karakter önerilir');
  } else {
    feedback.push('En az 6 karakter gerekli');
  }
